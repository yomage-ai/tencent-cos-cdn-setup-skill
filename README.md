...
如果是第一次摸索 COS + CDN + 私有桶完整链路，还是要花点时间来研究的，并且容易漏掉 CORS、私有回源、TypeA 鉴权、CNAME 或最小权限策略。使用这个 skill 后，常见流程会变成：3-10 分钟生成计划和操作指南，5-20 分钟执行或照着指南确认，剩余时间主要等待 CDN/DNS 生效。实际节省取决于账号状态、域名备案/解析、证书和权限审批。
...
如果只是试用，建议新建一个空文件夹作为配置工作区；如果你已经在开发项目，也可以直接在项目目录里打开已安装该 skill 的 AI 代理。这个 skill 默认会把计划、报告、状态和临时密钥文件写到独立运行目录，不会写进你的项目仓库。完成后，你只需要把报告里的配置值填回自己的项目。
